...
class Plant(pygame.sprite.Sprite):
    ...
    def grow(self):
        if self.check_watered(self.rect.center):
            ...
            # if the plant age is greater than 0
            if int(self.age) > 0:
                self.z = LAYERS['main']
...
class SoilLayer:
    def __init__(self, all_sprites, collision_sprites):
        ...
        # requirements of each of the tiles
        # if the area is cultivable
        # if the soil has been watered
        # if the soil has a plant already
        # Sounds for hoe action
    ...
    def create_soil_tiles(self):
        ...
        for index_row, row in enumerate(self.grid):
            for index_col, cell in enumerate(row):
                if 'X' in cell:
                    # tile options(what is in the top, left, right, bottom of current cell)
                    ...
                    if l and not any((t, r, b)):  # if there is tile on left side of current tile and not on right,
                        # top, bottom side.
                        tile_type = 'r'
